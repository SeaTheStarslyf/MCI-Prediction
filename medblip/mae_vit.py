...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, img, text_emb=None, mask_ratio=0.):
        ...
        for blk in self.blocks:
            x, attn = blk(x,text_emb)
            attns.append(attn)
        x = self.norm(x)
        

        return x, attns, mask, ids_restore
    ...
    def forward(self, imgs, text_emb=None, mask_ratio=0.75):
        latent, attn, mask, ids_restore = self.forward_encoder(imgs, text_emb, mask_ratio)
...
mae_vit_small_patch16 = mae_vit_small_patch16_dec512d8b  # decoder: 512 dim, 4 blocks
