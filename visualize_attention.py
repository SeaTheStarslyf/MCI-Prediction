...
# 生成柱状图
def generate_heatmap(attention_weights, feature_names, title, save_path, feature_sources=None, show_top_n=20):
    """生成并保存注意力柱状图"""
    # 调整图大小以适应竖轴特征名称
    ...
    if len(attention_weights) > show_top_n:
        # 选择注意力权重最高的前show_top_n个特征（降序排列）
        top_indices = np.argsort(attention_weights)[-show_top_n:][::-1]
        ...
        attention_weights = attention_weights[sorted_indices]
        feature_names = [feature_names[i] for i in sorted_indices]
        if feature_sources:
            feature_sources = [feature_sources[i] for i in sorted_indices]
    ...
    # 使用统一颜色绘制柱状图（水平方向）
    bars = ax.barh(y_pos, attention_weights, color='#4A90D9')
    
    # 设置y轴标签
    ax.set_yticks(y_pos)
    ax.set_yticklabels(feature_names)
    
    # 反转y轴，让重要的特征（权重最高）显示在上面
    ax.invert_yaxis()
    ...
    # 调整布局，确保标题位置正确
    plt.subplots_adjust(top=0.92, bottom=0.08, left=0.35, right=0.90)
...
# 生成综合柱状图（多模型平均）
def generate_combined_heatmap(all_attention_weights, feature_names, title, save_path, feature_sources=None, show_top_n=20):
    """生成并保存多模型平均的注意力柱状图"""
    # 调整图大小以适应竖轴特征名称
    ...
    if len(avg_attention) > show_top_n:
        # 选择平均注意力权重最高的前show_top_n个特征（降序排列）
        top_indices = np.argsort(avg_attention)[-show_top_n:][::-1]
        ...
        avg_attention = avg_attention[sorted_indices]
        std_attention = std_attention[sorted_indices]
        feature_names = [feature_names[i] for i in sorted_indices]
        if feature_sources:
            feature_sources = [feature_sources[i] for i in sorted_indices]
    ...
    # 使用统一颜色绘制柱状图（水平方向）
    bars = ax.barh(y_pos, avg_attention, color='#4A90D9')
    ...
    plt.title(f'{title}\n(Averaged over {len(all_attention_weights)} models)', fontsize=14, fontweight='bold', pad=20)
    if feature_sources:
        plt.ylabel('Features (Green: Cognitive, Blue: MRI)', fontsize=12)
    else:
        plt.ylabel('Features', fontsize=12)
    ...
    # 调整布局，确保标题位置正确
    plt.subplots_adjust(top=0.90, bottom=0.08, left=0.35, right=0.90)

    # 保存图表
    os.makedirs(os.path.dirname(save_path) if os.path.dirname(save_path) else '.', exist_ok=True)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"综合柱状图已保存到: {save_path}")
    plt.close()
